...
function RegistrationOtp(props) {
  ...
  const [errors, setError] = useState({name: '',seqNo: '',dob: '',regyr:'',gender:'',uid:'',certificateType:'',zone:'',district:'',sro:''})
  ...
  const handleValidation = () => {
    ...
    if (certificateType === undefined || certificateType === '') {
      formIsValid = false;
      errors.certificateType = 'This is a required field';
    }
    if (uid === undefined || uid === '') {
      formIsValid = false;
      errors.uid = 'This is a required field';
    }
  ...
  const handleSubmit=()=>{
    if(handleValidation()){
        if(formConsent === true){
              ...
              axios.post(`${BASE_URL}rg/mrctpdf`,body,{
                headers: {
                  Authorization: "Bearer " + sessionStorage.getItem("authtoken"),
                },
              }).then((res)=>{
                setLoader(false) 
                navigate('/OurServices/Register_document_MCert', { state: { data: res.data.data,
                  seqNo: seqNo,
                  regYear: regyr,
                  certiType: certificateType,
                  level1: zone,
          ...
          <div className="flex flex-col mb-10 ml-20 space-y-8" style={{marginLeft:'6%'}}>
            <div className='row'>
              ...
              <div className="font-bold col-md-6" id="enter">
                ...
                <input type="number" value={regyr} id="regyr" placeholder="Enter registration year" onChange={(e)=>setRegyr(e.target.value)} style={{border:'1px solid lightgrey',marginTop:'0.5rem',width:'80%',height:'40px',borderRadius:'7px',padding:'10px',fontSize:'15px',fontWeight:'500',color:'grey',background:'#f7fafc'}}/>
                { errors.regyr !== '' && errors.errors.regyr === "This is a required field" &&
                    <p className="text-xs" style={{color:'red'}}>{t("requiredField")}</p>}
              </div> 
              <div className="font-bold col-md-6" id="enter">
                <label style={{fontWeight:'500',fontSize:'15px'}}>Enter Certificate Type</label><br/>
                <input type="text" value={certificateType} id="regyr" placeholder="Enter certificate Type" onChange={(e)=>setcertificateType(e.target.value)} style={{border:'1px solid lightgrey',marginTop:'0.5rem',width:'80%',height:'40px',borderRadius:'7px',padding:'10px',fontSize:'15px',fontWeight:'500',color:'grey',background:'#f7fafc'}}/>
                { errors.certificateType !== '' && errors.errors.certificateType === "This is a required field" &&
                    <p className="text-xs" style={{color:'red'}}>{t("requiredField")}</p>}
              </div> 
              <div className="font-bold col-md-6" id="enter">
                <label style={{fontWeight:'500',fontSize:'15px'}}>Enter uid number</label><br/>
                <input type="text" value={uid} id="regyr" placeholder="Enter your uid" onChange={(e)=>setUid(e.target.value)} style={{border:'1px solid lightgrey',marginTop:'0.5rem',width:'80%',height:'40px',borderRadius:'7px',padding:'10px',fontSize:'15px',fontWeight:'500',color:'grey',background:'#f7fafc'}}/>
                { errors.uid !== '' && errors.errors.uid === "This is a required field" &&
                    <p className="text-xs" style={{color:'red'}}>{t("requiredField")}</p>}
              </div> 
              <div className="font-bold col-md-6" id="enter">
